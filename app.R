...
server <- function(input, output) {
  output$precommit <- renderUI({
    tagList(
      tags$h2("This repo"),
      tags$p(
        "This repo is to demonstrate some of the functionality of pre-commit for working with R and Python code. Check out the ",
      ...
      tags$h2("Pre-commit"),
      tags$p(
        tags$a("Pre-commit", href = "https://pre-commit.com/"),
      ...
      tags$p(
        "You first need to install the pre-commit software on your system. There are many ways to do this. See the pre-commit website for instructions:",
...
python -m pip install pre-commit"),
      ...
      tags$h3("Hooks"),
      tags$p(
        "You can find LOTS more hooks on the Pre-commit site(",
